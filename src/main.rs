...
fn main() {
    ...
    let left_uniform = Uniform::from(0..LEFT.len());
    let right_uniform = Uniform::from(0..RIGHT.len());

    let left_idx = left_uniform.sample(&mut rng);
    let right_idx = right_uniform.sample(&mut rng);
    print!("{}-{}", LEFT[left_idx], RIGHT[right_idx]);
}

const LEFT: &'static [&'static str] = &[
    "admiring",
    "adoring",
    "affectionate",
    "agitated",
